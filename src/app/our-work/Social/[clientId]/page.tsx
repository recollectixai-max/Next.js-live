...
import { getSocialClient } from "@/data/social-data";

// ─────────────────────────────────────────────────────────────────────────────
//  Social CLIENT DATA STORE
...
//  PAGE
// ─────────────────────────────────────────────────────────────────────────────
export default function SocialCaseStudy({
  params,
}: {
  params: Promise<{ clientId: string }>;
}) {
  const { clientId } = use(params);
  const client = SocialData[clientId] ?? SocialData[""];
  const [activeReel, setActiveReel] = useState(0);
  const reels = client.reels ?? [];
  const editorialGallery = client.editorialGallery ?? [];

  // const client = getSocialClient(clientId);
        ...
        <div className="header-grid">
          ...
          >
            ...
            <div className="info-item">
              ...
              <a href={client.SocialLink} target="_blank" rel="noopener noreferrer" className="info-link">
